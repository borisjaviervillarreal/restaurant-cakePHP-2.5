<?php
// debug($pedidos);
echo $this->Html->script(array('cart.js','jquery.animate-colors.js'),array('inline'=>false));
?>
...
<?php foreach($pedidos as $pedido): ?>
	...
	<div class="row" id="row-<?php echo $pedido['Pedido']['id']; ?>">
		...
		<div class="col col-sm-1">
			<?php echo $this->Form->input($pedido['Pedido']['id'], array('div' => false, 'class' => 'numeric form-control input-small', 'label' => false, 'size' => 2, 'maxlenght' => 2, 'tabindex' => $tabindex++, 'data-id' => $pedido['Pedido']['id'], 'data-id-us' => $pedido['Pedido']['user_id'], 'value' => $pedido['Pedido']['cantidad'])); ?>
		</div>
		...
		<div class="col col-sm-1">
			<?php
			echo $this->Html->link('<span class="glyphicon glyphicon-trash" aria-hidden="true"></span>', '#', array('escapeTitle' => false, 'title' => 'Eliminar item', 'id' => $pedido['Pedido']['id'],'data-id-use' => $pedido['Pedido']['user_id'], 'class' => 'remove'));
			?>
		</div>
	</div>
	<br />
...
<div class="row">
	<div class="col col-sm-12">
		...
		<?php echo $this->Html->link('Generar Orden', array('controller' => 'ordens', 'action' => 'add',$pedido['Pedido']['user_id']), array('class' => 'btn btn-primary')); ?>
